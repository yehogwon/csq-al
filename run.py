if __name__ == '__main__':
    ...
    from model import preact_resnet
    from model import preact_wideresnet
    from model import csvm

    from model import SUPPORT_PRETRAINED
    ...
    def main(cmd_args: argparse.Namespace):
        ...
        args = configs[cmd_args.dataset + cmd_args.config_suffix]

        args['cmd_args'] = cmd_args
        args['budget'] = budget
        args['seed'] = cmd_args.seed

        args['partial'] = cmd_args.partial
        args['partial_loss'] = []

        if cmd_args.negative_loss:
            args['partial_loss'].append('negative')
        
        args['resample'] = cmd_args.resample
        if args['resample'] and not args['partial']:
            raise ValueError('Resample is needed only with partial labels')
        
        args['lambda_negative'] = cmd_args.lambda_negative

        args['oracle_wrong_rate'] = cmd_args.oracle_wrong_rate
        args['wrong_within_candidate'] = cmd_args.wrong_within_candidate

        args['modelType'] = cmd_args.model
        args['pretrained'] = cmd_args.pretrained
        args['linear_probe'] = cmd_args.linear_probe
        args['svm_gpu'] = cmd_args.svm_gpu
        ...
        if args['saal_batch_size'] == 0 and args['loader_te_args'] is not None:
        ...
        args['language_backbone'] = cmd_args.language_backbone
        ...
        if cmd_args.pretrained:
            if cmd_args.model not in SUPPORT_PRETRAINED:
                raise ValueError(f'Pretrained is not supported for {cmd_args.model}')

        # load specified network
        if args['linear_probe'] and not args['pretrained']:
            raise ValueError('Linear probe is only supported for pretrained models')

        if cmd_args.model == 'resnet18':
            net_constructor = partial(resnet.ResNet18, n_classes=args['nClasses'])
        elif cmd_args.model == 'resnet34':
            net_constructor = partial(resnet.ResNet34, n_classes=args['nClasses'])
        elif cmd_args.model == 'resnet50':
            net_constructor = partial(resnet.ResNet50, n_classes=args['nClasses'], pretrained=cmd_args.pretrained, linear_probe=cmd_args.linear_probe)  # dino pretraine)
        elif cmd_args.model == 'resnet101':
            net_constructor = partial(resnet.ResNet101, n_classes=args['nClasses'])
        elif cmd_args.model == 'efficientnetb3': 
            net_constructor = partial(efficientnet.EfficientNetB3, n_classes=args['nClasses'], pretrained=cmd_args.pretrained, linear_probe=cmd_args.linear_probe)  # imagenet supervised pretraine)
        elif cmd_args.model == 'vits16':
            net_constructor = partial(vit.vits16, n_classes=args['nClasses'], pretrained=cmd_args.pretrained, linear_probe=cmd_args.linear_probe)
        elif cmd_args.model == 'vits8':
            net_constructor = partial(vit.vits8, n_classes=args['nClasses'], pretrained=cmd_args.pretrained, linear_probe=cmd_args.linear_probe)
        elif cmd_args.model == 'vitb16':
            net_constructor = partial(vit.vitb16, n_classes=args['nClasses'], pretrained=cmd_args.pretrained, linear_probe=cmd_args.linear_probe)
        elif cmd_args.model == 'vitb8':
            net_constructor = partial(vit.vitb8, n_classes=args['nClasses'], pretrained=cmd_args.pretrained, linear_probe=cmd_args.linear_probe)
        elif cmd_args.model == 'mobilenet': 
            net_constructor = partial(mobilenet.MobileNetV3s, n_classes=args['nClasses'])
        elif cmd_args.model == 'wrn-28-5': # WRN-28-5
            net_constructor = partial(wideresnet.wideresnet, depth=28, widen_factor=5, n_classes=args['nClasses'])
        elif cmd_args.model == 'wrn-36-1': # WRN-36-1
            net_constructor = partial(wideresnet.wideresnet, depth=36, widen_factor=1, n_classes=args['nClasses'])
        elif cmd_args.model == 'wrn-36-2': # WRN-36-2
            net_constructor = partial(wideresnet.wideresnet, depth=36, widen_factor=2, n_classes=args['nClasses'])
        elif cmd_args.model == 'wrn-36-5': # WRN-36-5
            net_constructor = partial(wideresnet.wideresnet, depth=36, widen_factor=5, n_classes=args['nClasses'])
        elif cmd_args.model == 'vgg':
            net_constructor = partial(vgg.VGG, 'VGG16', n_classes=args['nClasses'])
        elif cmd_args.model == 'preactresnet18':
            net_constructor = partial(preact_resnet.PreActResNet18, n_classes=args['nClasses'])
        elif cmd_args.model == 'preactwideresnet18':
            net_constructor = partial(preact_wideresnet.PreActWideResNet18, widen_factor=3, num_classes=args['nClasses'])
        elif cmd_args.model == 'svm': 
            if cmd_args.dataset != 'R52': 
                raise ValueError('SVM is only supported for R52 dataset')
            net_constructor = partial(csvm.CSVM, n_classes=args['nClasses'], seed=cmd_args.seed, verbose=cmd_args.verbose, svm_gpu=args['svm_gpu'])
        else: 
        ...
        args['net_constructor'] = net_constructor
        net = net_constructor()
        ...
        for round in range(cmd_args.start_round, cmd_args.n_rounds + 1):
            ...
            if cmd_args.start_round > 1 and round == cmd_args.start_round:
                ...
                if round == 1:
                    ...
                    # query
                    if args['resample']:
                        _idxs_lb_bak = strategy.idxs_lb.copy()
                        strategy.idxs_lb[strategy.partial_label_idxs] = 0
                        output = strategy.query(budget)
                        strategy.idxs_lb = _idxs_lb_bak
                    else:
                        output = strategy.query(budget)
                    q_idxs = output
                ...
                if len(cmd_args.save_idxs_lb) > 0:
                    ...
                    elif isinstance(strategy, DynamicTopKStrategy):
                        np.savez(idxs_lb_store_path, idxs_lb=strategy.idxs_lb, cur_k=strategy.cur_k)
                    else:
                        np.savez(idxs_lb_store_path, idxs_lb=strategy.idxs_lb)
            ...
            if cmd_args.wandb: 
                ...
                if hasattr(strategy, 'partial_labels'):
                    partial_labels: np.ndarray = strategy.partial_labels[strategy.idxs_lb]
                    num_labeled_samples = sum(strategy.idxs_lb)
                    num_possible_classes = partial_labels.sum(axis=1)
                    
                    num_full_labels = (num_possible_classes == 1).sum()
                    num_partial_labels = (num_possible_classes > 1).sum()
                    num_no_labels = (num_possible_classes == 0).sum()

                    assert num_no_labels == 0, 'Partial labels with no candidates exist'
                    assert num_full_labels + num_partial_labels == num_labeled_samples, 'The total number of labels mismatch'

                    wandb.log({
                        'full_label_count': num_full_labels,
                        'full_label_ratio': num_full_labels / num_labeled_samples,
                        'partial_label_count': num_partial_labels,
                        'partial_label_ratio': num_partial_labels / num_labeled_samples
                    })
    ...
    ### Query
    parser.add_argument('--partial', action='store_true', help='only one query to have partial labels')
    parser.add_argument('--resample', action='store_true', help='select labeled samples again to get full labels')
    parser.add_argument('--oracle_wrong_rate', type=float, default=0., help='Probability that oracle chooses wrong label within candidate set')
    parser.add_argument('--wrong_within_candidate', action='store_true', help='oracle makes mistakes only within candidate sets')
    ...
    ### Dataset
    parser.add_argument('--dataset', type=str.upper, required=True, choices=['MNIST', 'FASHIONMNIST', 'SVHN', 'CIFAR10', 'CIFAR100', 'IMAGENET', 'IMAGENET32', 'IMAGENET64', 'CIFAR100N', 'CIFAR100LT', 'R52', 'CUB', 'TINYIMAGENET', 'INATURALIST', 'PRODUCTS10K'], help='dataset name')
    parser.add_argument('--config_suffix', type=str, default='', help='suffix for config dict key')
    parser.add_argument('--download', action='store_true', help='download dataset')

    ### Dataset (advanced)
    parser.add_argument('--language_backbone', type=str, default='tfidf', choices=['tfidf', 'roberta'], help='backbone for language classification')
    ...
    parser.add_argument('--pretrained', action='store_true', help='whether to load pretrained model')  # TODO: this may be SSL or SL; too ambiguous
    parser.add_argument('--linear_probe', action='store_true', help='whether to use linear probe for pretrained model')
    parser.add_argument('--svm_gpu', action='store_true', help='use GPU-accelerated SVM from cuML')
    ...
    parser.add_argument('--scheduler', type=str, default='MultiStepLR', choices=['MultiStepLR', 'CosineAnnealingLR', 'ExponentialLR'], help='specify scheduler')
    parser.add_argument('--milestones', type=int, nargs='*', default=[], help='milestones for learning rate scheduler')
    parser.add_argument('--steps', type=int, default=0, help='steps to decay lr for ExponentialLR')
    ...
    parser.add_argument('--negative_loss', action='store_true', help='use negative loss for partial labels')
    parser.add_argument('--lambda_negative', type=float, default=0.1, help='balance factor for negative loss')
