...
configs = {
    ...
    'PRODUCTS10K': {  # low resolution (default)
        'nClasses': 10_000,
        'n_epochs': 30,  # original: 80 / but efficientnet-b3 converges early
        'lr': 3e-4, 
        'milestones': [20, 40, 60],
        'gamma': 0.1,
        'weight_decay': 1e-4,
        'transform': transforms.Compose([
            transforms.RandomHorizontalFlip(),
            transforms.Resize(256),  # for high resolution, use 512
            transforms.RandomCrop(224),  # for high resolution, use 448
            transforms.ToTensor(),
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
        ]),
        'loader_tr_args': {
        ...
        'transformTest': transforms.Compose([
            transforms.Resize(256),  # for high resolution, use 512
            transforms.CenterCrop(224),  # for high resolution, use 448
            transforms.ToTensor(),
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
        ])
    },
    'PRODUCTS10KHR': {  # high resolution
        'nClasses': 10_000,
        'n_epochs': 30,  # original: 80 / but efficientnet-b3 converges early
        'lr': 3e-4, 
        'milestones': [20, 40, 60],
