...
def ResNet34(n_classes: int=10):
    return ResNet(BasicBlock, [3,4,6,3], n_classes)

def ResNet50(n_classes: int=10, pretrained: bool=False, linear_probe: bool=False) -> EmbeddingWrapper[ResNet]:
    if pretrained:
        _model = torch.hub.load('facebookresearch/dino:main', 'dino_resnet50')
        _model.emb_dim = 512 * Bottleneck.expansion
...
def ResNet101(n_classes: int=10):
    return ResNet(Bottleneck, [3,4,23,3], n_classes)

def ResNet152(n_classes: int=10):
